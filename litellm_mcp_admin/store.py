...
def _as_operations(value: Any) -> Any:
    ...
    ``woow_litellm_mcp_server.gating._normalize_operations`` accepts both.
    """
    if not value:
        return {}
    if isinstance(value, Mapping):
...
def env_from_tool_settings(tools: dict[str, Any]) -> dict[str, str]:
    ...
    ``"litellm_token_counter"`` both raised ``SettingsError`` at import time and
    ...
    return {
        "LITELLM_MCP_READONLY": "true" if settings.get("readonly") else "false",
        "LITELLM_MCP_DISABLED_CATEGORIES": json.dumps(
            _as_str_list(settings.get("disabled_categories"))
        ),
        "LITELLM_MCP_DISABLED_TOOLS": json.dumps(
            _as_str_list(settings.get("disabled_tools"))
        ),
        "LITELLM_MCP_DISABLED_OPERATIONS": json.dumps(
