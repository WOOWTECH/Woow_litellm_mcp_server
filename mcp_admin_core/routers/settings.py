...
*   **Every read is masked.**  ``GET /api/settings`` and
    ...
    LiteLLM master key, the admin password or the connector's ``mcp_auth_token``
...
*   **Every write is a MERGE.**  ``PUT /api/settings/{section}`` patches the
    ...
    ``litellm_mcp_master_key`` and then restarted the child without it, i.e. the
...
class ConnectionConfig(BaseModel):
    """Upstream service connection settings (LiteLLM gateway).

    For LiteLLM the connection carries ``litellm_mcp_base_url`` and
    ``litellm_mcp_master_key`` (a single Bearer secret, no key/secret
    pair).  Kept generic so the routers use whatever keys are present.
    """
...
def _merge_preserving_secrets(stored: dict[str, Any], updates: dict[str, Any], *, section: str) -> dict[str, Any]:
    ...
    * A partial PUT (the GUI saving one field) dropping every key it did not
      send — that is how ``litellm_mcp_master_key`` used to vanish.
...
def _history_entry(token: str) -> dict[str, str]:
    ...
    Exactly the shape ``litellm_mcp_admin.routers.tokens`` writes and the React
